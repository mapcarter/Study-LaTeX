...
\section{函数图}
\subsection{二维函数图}
绘制函数$f(x)=sin(x)$和$g(x)=cos(x)$在定义域$[-2\pi,2\pi]$上的图像。对于函数$f(x)=sin(x)$，我们使用$ red $和$ /tikz/line width={1pt} $来使其更显眼。\LaTeX 默认取点少，我们使用$samples=x$来控制取点数，使函数图像更为平滑。
